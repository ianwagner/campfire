...
const CopyRecipePreview = ({
  ...
  const addRecipeRow = () => {
    ...
    setResults((prev) => [
      ...prev,
      { recipeNo: prev.length + 1, components: blank, primaryText: '', headline: '', description: '', assets: [] },
    ]);
  };
  ...
  const handleEditRow = (idx) => {
    if (editing === idx) {
      const arr = [...results];
      arr[idx].primaryText = editPrimaryText;
      arr[idx].components = { ...editComponents };
      arr[idx].headline = editHeadline;
      arr[idx].description = editDescription;
      setResults(arr);
      setEditing(null);
    } else {
      setEditing(idx);
      setEditHeadline(results[idx].headline);
      setEditDescription(results[idx].description);
      setEditPrimaryText(results[idx].primaryText);
  ...
  const columnMeta = useMemo(() => {
    ...
    if (orderedComponents.length > 0) {
      ...
      cols.push({ key: "primaryText", label: "Primary Text" });
      cols.push({ key: "headline", label: "Headline" });
  ...
  useEffect(() => {
    setVisibleColumns((prev) => {
      ...
      Object.keys(updated).forEach((k) => {
        if (!(columnMeta.some((c) => c.key === k) || k === 'recipeNo' || ['primaryText','headline','description'].includes(k))) {
          delete updated[k];
        }
      });
      return updated;
              ...
              <h3 className="mb-2 font-semibold">Visible Columns</h3>
                ...
                <label className="block whitespace-nowrap">
                  <input
                    type="checkbox"
                    className="mr-1"
                    checked={visibleColumns["primaryText"] || false}
                    onChange={() =>
                      setVisibleColumns({
                        ...visibleColumns,
                        primaryText: !visibleColumns["primaryText"],
                      })
                    }
                  />
                  Primary Text
                ...
                <label className="block whitespace-nowrap">
                  <input
                    type="checkbox"
                    className="mr-1"
                    checked={visibleColumns["headline"] || false}
                    onChange={() =>
                      setVisibleColumns({
                        ...visibleColumns,
                        headline: !visibleColumns["headline"],
                      })
                    }
                  />
                  Headline
                ...
                <label className="block whitespace-nowrap">
                  <input
                    type="checkbox"
                    className="mr-1"
                    checked={visibleColumns["description"] || false}
                    onChange={() =>
                      setVisibleColumns({
                        ...visibleColumns,
          ...
          <table className="ad-table min-w-full table-auto text-sm">
            <thead>
              <tr>
                ...
                {visibleColumns["primaryText"] && <th className="w-80">Primary Text</th>}
                {visibleColumns["headline"] && <th className="w-80">Headline</th>}
                {visibleColumns["description"] && <th className="w-80">Description</th>}
                  ...
                  {visibleColumns["primaryText"] && (
                    <td className="whitespace-pre-wrap break-words w-80 align-middle copy-cell">
                      {editing === idx ? (
                        ...
                        <div className="min-h-[1.5rem] w-full" onClick={() => navigator.clipboard.writeText(r.primaryText)}>
                          {r.primaryText}
                        </div>
                      )}
                    </td>
                  ...
                  {visibleColumns["headline"] && (
                    <td className="whitespace-pre-wrap break-words w-80 align-middle copy-cell">
                      {editing === idx ? (
                        ...
                        <div className="min-h-[1.5rem] w-full" onClick={() => navigator.clipboard.writeText(r.headline)}>
                          {r.headline}
                        </div>
                      )}
                    </td>
                  ...
                  {visibleColumns["description"] && (
                    <td className="whitespace-pre-wrap break-words w-80 align-middle copy-cell">
                      {editing === idx ? (
                        ...
                        <div className="min-h-[1.5rem] w-full" onClick={() => navigator.clipboard.writeText(r.description)}>
                          {r.description}
                        </div>
                      )}
                    </td>
