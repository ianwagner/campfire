...
const RecipePreview = ({
  ...
  const handleBriefFilesChange = (e) => {
    ...
    setBriefFiles(combined);
    if (combined.length > 0) setDirty(true);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filtered =
          allowedTypeIds && allowedTypeIds.length
            ? typeList.filter((t) => allowedTypeIds.includes(t.id))
            : typeList;
        setTypes(filtered);
