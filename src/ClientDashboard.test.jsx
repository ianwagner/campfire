...
test('filters groups to public visibility when requested', async () => {
  const groupSnap = {
    docs: [
      {
        id: 'g1',
  ...
  render(
    <MemoryRouter>
      <ClientDashboard user={{ uid: 'u1' }} brandCodes={['B1']} publicOnly />
    </MemoryRouter>
  );

  expect(await screen.findByText('Public Group')).toBeInTheDocument();
