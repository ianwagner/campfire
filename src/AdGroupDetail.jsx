...
const AdGroupDetail = () => {
  ...
  const { role: userRole, brandCodes: userBrandCodes } = useUserRole(
  ...
  useEffect(() => {
    ...
    if (
      Array.isArray(userBrandCodes) &&
      userBrandCodes.length > 0 &&
      !userBrandCodes.includes(group.brandCode)
    ) {
      navigate(backPath, { replace: true });
    }
  }, [backPath, group?.brandCode, isClient, navigate, userBrandCodes]);
  ...
  const renderClientView = () => {
    const heroAsset = pickHeroAsset(assets);
    const heroPreview =
      heroAsset?.thumbnailUrl ||
      heroAsset?.firebaseUrl ||
      heroAsset?.cdnUrl ||
      '';
    const heroIsVideo = heroPreview ? isVideoUrl(heroPreview) : false;
    ...
    const lastUpdatedDate =
      toDateValue(group.lastUpdated) || toDateValue(group.createdAt);
    const formattedLastUpdated = lastUpdatedDate
      ? lastUpdatedDate.toLocaleString(undefined, {
          month: 'short',
          day: 'numeric',
          year: 'numeric',
          hour: 'numeric',
      ...
      : null;
    const totalAds = assets.length;
    const approvedAds = assets.filter((a) => a.status === 'approved').length;
    const readyAds = assets.filter((a) => a.status === 'ready').length;
    const sortedAssets = [...assets].sort((a, b) => {
      const aRecipe =
        a.recipeCode || parseAdFilename(a.filename || '').recipeCode || '';
      const bRecipe =
        b.recipeCode || parseAdFilename(b.filename || '').recipeCode || '';
      if (aRecipe !== bRecipe) return aRecipe.localeCompare(bRecipe);
      return (a.filename || '').localeCompare(b.filename || '');
    });
    const sortedBriefAssets = [...briefAssets].sort((a, b) =>
      (a.filename || '').localeCompare(b.filename || '')
    );
    const copyColumns = [
      { key: 'primary', label: 'Platform' },
      { key: 'headline', label: 'Headline' },
      { key: 'description', label: 'Description' },
      { key: 'cta', label: 'CTA' },
    ];
    const resolveDownloadUrl = (asset) =>
      asset?.downloadUrl ||
      asset?.firebaseUrl ||
      asset?.cdnUrl ||
      asset?.thumbnailUrl ||
    ...
    return (
      <div className="min-h-screen p-4">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="bg-white dark:bg-[var(--dark-sidebar-bg)] shadow-sm rounded-lg p-6">
            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
              <div className="text-sm text-gray-600 dark:text-gray-300 space-y-1">
                <div>Brand: {brandName || group.brandCode}</div>
              ...
              <div className="flex gap-3">
                <div className="text-center">
                  ...
                  <p className="text-lg font-semibold">{totalAds}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">
                    Ready
                  </p>
                  <p className="text-lg font-semibold">{readyAds}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">
                    Approved
                  </p>
                  <p className="text-lg font-semibold">{approvedAds}</p>
                </div>
              </div>
            </div>
            {heroPreview && (
              <div className="mt-6 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 bg-black/5">
                {heroIsVideo ? (
                  <VideoPlayer
                    src={heroPreview}
                    poster={heroAsset?.thumbnailUrl}
                    className="w-full max-h-[22rem] object-contain"
                  />
                ) : (
                  <OptimizedImage
                    pngUrl={heroPreview}
                    alt={heroAsset?.filename || group.name || 'Ad preview'}
                    className="w-full max-h-[22rem] object-contain"
                  />
                )}
              </div>
          ...
          <section className="bg-white dark:bg-[var(--dark-sidebar-bg)] shadow-sm rounded-lg p-6">
            ...
            {group.notes ? (
              ...
              <p className="text-sm text-gray-500">No brief notes available.</p>
            )}
            {sortedBriefAssets.length > 0 && (
              <div className="mt-4 space-y-3">
                {sortedBriefAssets.map((asset) => {
                  const downloadHref = resolveDownloadUrl(asset);
                  return (
                    <div
                      key={asset.id}
                    ...
                    >
                      ...
                      {downloadHref ? (
                        <a
                          href={downloadHref}
                          download
                          className="btn-secondary px-3 py-1 text-sm inline-flex items-center gap-1"
                        >
                          <FiDownload />
                          Download
                        </a>
          ...
          <section className="bg-white dark:bg-[var(--dark-sidebar-bg)] shadow-sm rounded-lg p-6">
            <h2 className="text-lg font-semibold mb-3">Platform Copy</h2>
            {copyCards.length > 0 ? (
              <div className="overflow-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {copyCards.map((card) => (
                      <tr key={card.id} className="border-b border-gray-100 dark:border-gray-800 align-top">
                        {copyColumns.map((column) => {
                          const value = card[column.key];
                          return (
                            <td key={column.key} className="py-2 pr-4">
                              {value ? (
                                <span className={column.key === 'description' ? 'whitespace-pre-line' : ''}>
                                  {value}
                                </span>
                              ) : (
              ...
              <p className="text-sm text-gray-500">No platform copy has been added yet.</p>
            )}
          </section>
          ...
          <section className="bg-white dark:bg-[var(--dark-sidebar-bg)] shadow-sm rounded-lg p-6">
            ...
            {sortedAssets.length > 0 ? (
                ...
                <table className="w-full text-sm min-w-[40rem]">
                  ...
                  <tbody>
                    {sortedAssets.map((asset) => {
                      const recipeCode =
                        asset.recipeCode ||
                        parseAdFilename(asset.filename || '').recipeCode ||
                        '—';
                      const aspectRatio =
                        asset.aspectRatio ||
                        asset.ratio ||
                      ...
                      return (
                        <tr key={asset.id} className="border-b border-gray-100 dark:border-gray-800 align-top">
                          <td className="py-2 pr-4">
                            <div className="w-28 h-28 bg-gray-100 dark:bg-gray-900 rounded-md overflow-hidden flex items-center justify-center">
                              {previewUrl ? (
                                previewIsVideo ? (
                                  <VideoPlayer
                                    src={previewUrl}
                          ...
                          <td className="py-2 pr-4 align-top">
                            <div className="font-medium">{recipeCode}</div>
                            <div className="text-xs text-gray-500 break-all">
                              {asset.filename}
                            </div>
                          </td>
                          <td className="py-2 pr-4 align-top">{aspectRatio}</td>
                          <td className="py-2 pr-4 align-top capitalize">{asset.status || 'pending'}</td>
              ...
              <p className="text-sm text-gray-500">No ads have been uploaded yet.</p>
            )}
          </section>
        </div>
      </div>
