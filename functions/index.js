...
export const applyPublicReviewUpdate = onDocumentCreated(
  ...
  async (event) => {
    ...
    for (const [key, value] of Object.entries(update)) {
      ...
      if (key === 'clientNoteTimestamp') {
        if (value === null || value === undefined || value === '') {
          resolvedUpdate.clientNoteTimestamp = null;
        } else {
          const parsed = new Date(value);
          if (!Number.isNaN(parsed.getTime())) {
            resolvedUpdate.clientNoteTimestamp = admin.firestore.Timestamp.fromDate(
              parsed
            );
          }
        }
      ...
      if (key === 'lastUpdated') {
        resolvedUpdate.lastUpdated = admin.firestore.FieldValue.serverTimestamp();
        continue;
      }

      if (key === 'completedAt') {
        if (value === null) {
          resolvedUpdate.completedAt = null;
        }
        continue;
      }

      resolvedUpdate[key] = value;
    }

    if (Object.prototype.hasOwnProperty.call(resolvedUpdate, 'status')) {
      const normalized = String(resolvedUpdate.status || '')
        .trim()
        .toLowerCase();
      resolvedUpdate.lastUpdated = admin.firestore.FieldValue.serverTimestamp();
      if (['done', 'reviewed'].includes(normalized)) {
        resolvedUpdate.completedAt = admin.firestore.FieldValue.serverTimestamp();
      } else if (!Object.prototype.hasOwnProperty.call(resolvedUpdate, 'completedAt')) {
        resolvedUpdate.completedAt = null;
      }
    }
    ...
    try {
      await groupRef.update(resolvedUpdate);
      await updateSnap.ref.update({
        processedAt: admin.firestore.FieldValue.serverTimestamp(),
      });
    } catch (err) {
      console.error('Failed to apply public review update', {
        adGroupId: groupId,
        updateId: updateSnap.id,
        error: err,
      });
    }
